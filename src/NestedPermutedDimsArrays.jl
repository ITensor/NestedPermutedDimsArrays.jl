...
struct NestedPermutedDimsArray{T, N, perm, iperm, AA <: AbstractArray} <:
    AbstractArray{T, N}
    parent::AA
    ...
    function NestedPermutedDimsArray{T, N, perm, iperm, AA}(
        ...
        isperm(perm) ||
            throw(
            ArgumentError(
                string(perm, " is not a valid permutation of dimensions 1:", N)
            )
        )
...
function Base.elsize(
        ::Type{<:NestedPermutedDimsArray{<:Any, <:Any, <:Any, <:Any, P}}
    ) where {P}
...
@inline genperm(I::NTuple{N, Any}, perm::Dims{N}) where {N} =
...
function Base.mapreducedim!(
        f, op::CommutativeOps, B::AbstractArray{T, N},
